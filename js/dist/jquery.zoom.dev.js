...
jQuery(function ($) {
  $.fn.zoom = function (options) {
    var addHTML = '<div class="jZoom"><div class="zTerritory"></div><div class="zImage"></div></div>';
    var setting = {
      toppage: true,
      par: 10,
      initial: 100,
    ...
    return $(this).each(function (i) {
      var C_MAX_HEIGHT, C_MAX_WIDTH, canvas_height, canvas_width, jZoom, territory, obj;
      ...
      function zoom(img, e, x, y) {
        ...
        setPos(img, e, x, y); //setScaleBar(img,e);
      ...
      function drawImg(obj, zoom_asr, src) {
        var o_width, o_height, d_top, d_left, d_width, d_height, f_top, f_left, asr_diff;
        ...
        obj.width(o_width).height(o_height).css({
          top: f_top,
          left: f_left
        }).html(source);
      }
      ...
      function toToppage() {
        $(".arw_left,.arw_right,.arw_top,.arw_bottom").fadeOut();
        drawImg(obj, S_asr, "img/main/main.jpg");
        zoom_e = 500;
        zoom(obj, zoom_e);
        setBacktopPos();
      ...
      function arrowFade() {
        ...
        console.log("top:", hasTop, " right:", hasRight, " left:", hasLeft, " bottom:", hasBottom);
      ...
      function linkNext() {
        ...
        if (hasRight) {
          if (left2 <= canvas_width + 10 && dPosX >= 2 || left2 <= 1000 && mPosX >= 20) {
        ...
        if (hasBottom) {
          if (top2 <= canvas_height + 10 && dPosY >= 2 || top2 <= canvas_height + 10 && mPosY >= 20) {
      ...
      obj.draggable({
        ...
        stop: function stop(e, ui) {
          ...
          mPosY = mStartY - e.screenY; //console.log(dPosX + ' ' + dPosY + '   ' + mPosX + ' ' + mPosY);
      ...
      function touchHandlerStart(event) {
        ...
        scaleStart = Math.sqrt(Math.pow(event.touches[1].pageX - event.touches[0].pageX, 2) + Math.pow(event.touches[1].pageY - event.touches[0].pageY, 2));
      }
      ...
      function touchHandlerMove(event) {
        ...
        var x = Math.sqrt(Math.pow((event.touches[1].pageX - event.touches[0].pageX) / 2, 2));
        var y = Math.sqrt(Math.pow((event.touches[1].pageY - event.touches[0].pageY) / 2, 2));
        scaleMove = Math.sqrt(Math.pow(event.touches[1].pageX - event.touches[0].pageX, 2) + Math.pow(event.touches[1].pageY - event.touches[0].pageY, 2));
        ...
        zoom_e = zoom($(".zImage"), zoom_e, event.touches[0].pageX, event.touches[0].pageY);
        arrowFade();
